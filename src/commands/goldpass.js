...
module.exports = {
  ...
  async execute(interaction) {
    function convertToIso(dateString) {
      ...
      const isoDate = `${datePart.slice(0, 4)}-${datePart.slice(
        4,
        6,
      )}-${datePart.slice(6)}`;

      // Insert colons into the time part
      const isoTime = `${timePart.slice(0, 2)}:${timePart.slice(
        2,
        4,
      )}:${timePart.slice(4, 6)}${timePart.slice(6)}`;
      ...
      .then(async (res) => {
        if (res.status == 200) {
          ...
          clashembed
            ...
            .setColor("#ffc147")
            .setThumbnail(
              "https://imgs.search.brave.com/P_VWVYXFAR0gXiGzoyAru6j-z7W51w9pd1D5hHErxok/rs:fit:400:400:1/g:ce/aHR0cHM6Ly9pMS53/cC5jb20vZ2FtaW5n/Z2VhcnNuZXBhbC5j/b20vd3AtY29udGVu/dC91cGxvYWRzLzIw/MTkvMTIvZ29sZC1w/YXNzLnBuZz9maXQ9/NDAwJTJDNDAwJnNz/bD0x",
            )
            .setDescription(
